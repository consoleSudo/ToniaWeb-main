// api/save-content.js
// Vercel Serverless Function — securely saves content.json changes to GitHub via the GitHub API.
...
//   GITHUB_REPO     – e.g. "consoleSudo/ToniaWeb-main"
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!rawBody || rawBody.length > 50 * 1024 * 1024) { // 50MB max
  ...
  if (!isValidContent(content)) {
    return res.status(400).json({ error: 'Invalid content structure. Refresh and try again.' });
  }
  ...
  try {
    ...
    if (currentFile.status !== 200) {
      return res.status(500).json({
        error: `Could not fetch content.json from GitHub: ${currentFile.body.message || 'Not found'}`
      });
    }
